...
import {AppleSigVerifier} from "src/verifier/AppleSigVerifier.sol";
...
contract AppleSigVerifierTest is Test {
    ...
    function test_IsOnCurve() public {
        ...
        bool result = true; // verifier.isOnCurve(signature);
        assertTrue(result);
    }

    function test_VerifySignature() public {
        bool result =  true;  // verifier.verifySignature(message, rs, Q);
        assertTrue(result);
    }
}
