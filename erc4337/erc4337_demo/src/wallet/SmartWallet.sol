...
import {UserOperation} from "../interface/UserOperation.sol";
...
contract SmartWallet is Ownable, IAccount {
    ...
    function validateUserOp(
        ...
        // Validate signature
        _validateSignature(userOp, userOpHash);  // 校验签名   userOpHash 不是通过 keccak256(abi.encode(userOp.hash(), address(this), block.chainid)); 可以算出来吗 
        ...
        if (userOp.initCode.length == 0) {
            ...
            require(currentNonce == userOp.nonce, "SmartWallet: Invalid nonce");  // 对nounce 进行校验  避免双花 

            // Effects
            // Increment nonce
            _updateNonce();  // 更新 nounce  
        }

        // Interactions
        _prefundEntryPoint(missingWalletFunds);  // 转钱给 entrypoint 合约 
        return 0;  //始终返回0 
    }
    ...
    function withdrawERC20(address token, address to, uint256 amount) external onlyOwner {
     ...
     function transferMoney(address token, address to, uint256 amount) external onlyEntryPoint {
        // console2.log("----transferMoney---token=" , token);
        // console2.log("----transferMoney---to=" , to);
        // console2.log("----transferMoney---amount=" , amount);
        SafeERC20.safeTransfer(IERC20(token), to, amount);
        emit WithdrawERC20(to, token, amount);
    }
    ...
    /// @notice Validate the signature of the userOperation  通过双椭圆曲线数字签名算法 校验   查看 readMe.md 文档 
