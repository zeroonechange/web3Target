...
import {UserOperation, UserOperationLib} from "../interface/UserOperation.sol";
...
import {ICreate2Deployer} from "../interface/ICreate2Deployer.sol";
...
contract EntryPoint is IEntryPoint, StakeManager {
    ...
    function handleOps(UserOperation[] calldata ops, address payable beneficiary) public {
        ...
        unchecked {
            for (uint256 i = 0; i < opslen; i++) {
                 // 1. 用 双椭圆曲线数字签名算法  校验签名是否通过    2. 校验 paymaster 是否有足够的 stake 和 deposite 用于支付 gas 费 (默认通过)
        ...
        returns (uint256 actualGasCost)
    {
        ...
        if (callData.length > 0) {
            // 这个 sender 就是 walletFactory.computeAddress(address(entryPoint), walletOwner, salt); 
            // 得到的  就是 SmartWallet 的地址    也就是 通过 callData  调用 里面任意的函数  包括转账 
    ...
    // (callGasLimit + verificationGasLimit * 3 + preVerificationGas) *
    ...
    function _createSenderIfNeeded(uint256 opIndex, MemoryUserOp memory mUserOp, bytes calldata initCode) internal {
        if (initCode.length != 0) {
            if (mUserOp.sender.code.length != 0) {
                return;  // 合约创建了就不要再创建了
                // revert FailedOp(opIndex, address(0), "sender already constructed");
            }
            address sender1 = senderCreator.createSender(initCode); // 创建合约
            ...
            if (sender1.code.length == 0) {
                revert FailedOp(opIndex, address(0), "initCode failed to create sender");
            }
        }
    }
    ...
    /**
     * call account.validateUserOp.  调用 账号去执行校验过程
    ...
    function _validateAccountPrepayment(
        ...
        unchecked {
            uint256 preGas = gasleft();
            MemoryUserOp memory mUserOp = opInfo.mUserOp;
            _createSenderIfNeeded(opIndex, mUserOp, op.initCode); // 创建合约 如果initCode存在的话
            ...
            ) returns (uint256 _deadline) {  // 返回时间期限   参考 SmartWallet.validateUserOp() 的逻辑实现  这里始终返回0 
                // solhint-disable-next-line not-rely-on-time
                if (_deadline != 0 && _deadline < block.timestamp) {
                    revert FailedOp(opIndex, address(0), "expired");
                }
                deadline = _deadline;  // 为0  
     ...
     */
    // 如果有 paymaster   校验有足够的存款和stake  用于支付 gas 费   这里直接跳过了  不需要计算了
    function _validatePaymasterPrepayment( 
        ...
        unchecked {
            ...
            // 实际的代码没有做任何校验  直接过了  返回 return ("", 0);   所以这里不需要care 
    ...
    function _validatePrepayment(
        ...
        UserOpInfo memory outOpInfo,
        address aggregator // 暂时不用聚合器  不然逻辑太复杂
    ) private returns (address actualAggregator, uint256 deadline) {
        uint256 preGas = gasleft();
        MemoryUserOp memory mUserOp = outOpInfo.mUserOp;
        _copyUserOpToMemory(userOp, mUserOp); // 把参数复制给 UserOpInfo
        // 为啥还要算一下 哈希?    之前构建 signature 的时候  就算了一遍 userOpHash = keccak256(abi.encode(userOp.hash(), address(this), block.chainid));
        //                       然后签名  signature = sign(privateKey, ECDSA.toEthSignedMessageHash(msgHash)) -> (r,s,v) -> r+s+v
        ...
        uint256 gasUsedByValidateAccountPrepayment;
        uint256 requiredPreFund = _getRequiredPrefund(mUserOp); // 计算 gas 消耗后还能退款多少
        (
            gasUsedByValidateAccountPrepayment, // 校验账号预付花了多少gas
            actualAggregator, // 真正的聚合器  这里没有用
            deadline //校验通过 总是为0   0 for valid signature, 1 to mark signature failure,
        ) = _validateAccountPrepayment(opIndex, userOp, outOpInfo, aggregator, requiredPreFund);  // 1. 用 双椭圆曲线数字签名算法  校验签名是否通过
        //a "marker" where account opcode validation is done and paymaster opcode validation is about to start
        // (used only by off-chain simulateValidation)
        numberMarker();  //  

        bytes memory context;
        if (mUserOp.paymaster != address(0)) {  // 打钱地址 用来支付 gas费
            uint256 paymasterDeadline;  // 直接过了  不校验了 
            // 2. 校验 paymaster 是否有足够的 stake 和 deposite 用于支付 gas 费 
